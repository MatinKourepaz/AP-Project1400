...
HardForm = uic.loadUiType(os.path.join(os.getcwd(), "Hard.ui"))[0]


class HardWindow(QMainWindow, HardForm):
    def __init__(self,):
        super(HardWindow, self).__init__()
        self.WIN = False
        self.LOSS = False
        self.RandArr = [randint(1, 8) for i in range(8)]
        self.currentRow = [0, 0, 0, 0, 0, 0, 0, 0]
        self.row = 1
        self.column = 1
        self.setupUi(self)
    ...
    def game_reset(self):
        ...
        self.Color5.setStyleSheet(Cstyle)
        self.Color6.setStyleSheet(Cstyle)
        self.Color7.setStyleSheet(Cstyle)
        self.Color8.setStyleSheet(Cstyle)
    ...
    def check(self):
        if self.WIN == False and self.LOSS == False and self.currentRow[7] != 0:
            red_hint = 0
            RAcopy = self.RandArr.copy()

            for i, j, k in zip(self.RandArr, self.currentRow, range(8)):
                if i == j:
                    red_hint += 1
            ...
            for j, k in zip(self.currentRow, range(8)):
