...
MediumForm = uic.loadUiType(os.path.join(os.getcwd(), "Medium.ui"))[0]


class MediumWindow(QMainWindow, MediumForm):
    def __init__(self,):
        super(MediumWindow, self).__init__()
        self.WIN = False
        self.LOSS = False
        self.RandArr = [randint(1, 8) for i in range(6)]
        self.currentRow = [0, 0, 0, 0, 0, 0]
        self.row = 1
        self.column = 1
        self.setupUi(self)
    ...
    def check(self):
        if self.WIN == False and self.LOSS == False and self.currentRow[5] != 0:
            red_hint = 0
            RAcopy = self.RandArr.copy()

            for i, j, k in zip(self.RandArr, self.currentRow, range(6)):
                if i == j:
                    red_hint += 1
            ...
            for j, k in zip(self.currentRow, range(6)):
